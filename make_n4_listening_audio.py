# -*- coding: utf-8 -*-
"""
N4 listening audio generator (edge-tts only; no ffmpeg / no pydub).

★N4の音声を作ります。間の長さは**級によらず共通**の値です（kijun.json の _音声の間_共通）。
   ...
   模擬試験モードを作るときは n4_audio_timing.json の「本試験どおり」の値を使ってください。

Reads n4_listening_manifest.json and, for each question, synthesizes each line
with edge-tts, then joins the MP3 parts by binary concatenation into
  audio/n4_qX_YY.mp3

Incremental / auto-update:
  A small signature file (audio/_n4_audio_sig.json) records a hash of each
...
PREFIX_KIND = {"n4_q1":"課題理解", "n4_q2":"ポイント理解",
               "n4_q3":"発話表現", "n4_q4":"即時応答"}
...
async def main():
    ...
    if not os.path.exists(MANIFEST):
        log("[ERROR] n4_listening_manifest.json not found next to this script.")
        sys.exit(1)

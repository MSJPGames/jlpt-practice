...
# -*- coding: utf-8 -*-
"""
n1_listening_manifest.json に新しい24問（n1_q1_47〜54・n1_q3_41〜56）を書き足す。

つかいかた:
    python add_new_to_manifest.py

このスクリプトと n1_listening_manifest_add.json を
n1_listening_manifest.json と同じフォルダ（JLPT PRACTICE）に置いて実行してください。

・既存の273問には一切さわりません（署名が変わらないので音声も作り直されません）。
・実行前に n1_listening_manifest.json.bak を作ります。
・すでに同じ名前の項目があれば入れ替えます（二重登録になりません）。
...
NEW = os.path.join(HERE, 'n1_listening_manifest_add.json')
...
def main():
    ...
    if not os.path.exists(NEW):
        die('n1_listening_manifest_add.json が見つかりません。ZIPの中の1つです。')
    ...
    # ── 同名の項目があれば取り除く ───────────────────────────
    def is_new(e):
        return (isinstance(e, dict) and isinstance(e.get(fkey), str)
                and re.search(r'n1_q1_(4[7-9]|5[0-4])\.mp3$|n1_q3_(4[1-9]|5[0-6])\.mp3$', e[fkey]))

    before = len(entries)
    kept = [e for e in entries if not is_new(e)]

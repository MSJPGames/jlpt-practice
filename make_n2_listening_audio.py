# -*- coding: utf-8 -*-
"""
N2 listening audio generator (edge-tts only; no ffmpeg / no pydub).

★N2の音声を作ります。間の長さは公式CD（N2Q1〜Q5）を実測して決めた値です。
   ...
   模擬試験モードを作るときは n2_audio_timing.json の「本試験どおり」の値を使ってください。

Reads n2_listening_manifest.json and, for each question, synthesizes each line
with edge-tts, then joins the MP3 parts by binary concatenation into
  audio/n2_qX_YY.mp3

Incremental / auto-update:
  A small signature file (audio/_n2_audio_sig.json) records a hash of each
...
PREFIX_KIND = {"n2_q1":"課題理解", "n2_q2":"ポイント理解", "n2_q3":"概要理解",
               "n2_q4":"即時応答", "n2_q5":"統合理解"}
...
async def main():
    ...
    if not os.path.exists(MANIFEST):
        log("[ERROR] n2_listening_manifest.json not found next to this script.")
        sys.exit(1)

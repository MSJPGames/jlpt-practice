# -*- coding: utf-8 -*-
"""
N5 listening audio generator (edge-tts only; no ffmpeg / no pydub).

★N5の音声を作ります。間の長さは**級によらず共通**の値です（kijun.json の _音声の間_共通）。
   N2とN3の公式CDを実測した結果、ちがいは最大0.2秒だったので、級で分けないことにしました。
   ...
   模擬試験モードを作るときは n5_audio_timing.json の「本試験どおり」の値を使ってください。

Reads n5_listening_manifest.json and, for each question, synthesizes each line
with edge-tts, then joins the MP3 parts by binary concatenation into
  audio/n5_qX_YY.mp3

Incremental / auto-update:
  A small signature file (audio/_n5_audio_sig.json) records a hash of each
...
MA = {
    ...
    # 発話表現・即時応答は選択肢を音声で読む。3肢の大問は肢の間を短めにする
    "発話表現":     {"after_intro":2.0, "before_question":None,"choice_first":2.1,  "choice_gap":2.1},
    "即時応答":     {"after_intro":None,"before_question":None,"choice_first":2.0,  "choice_gap":2.0},
}
# ファイル名から大問が分からないときの予備（file_to_daimon.json が無い場合）
PREFIX_KIND = {"n5_q1":"課題理解", "n5_q2":"ポイント理解",
               "n5_q3":"発話表現", "n5_q4":"即時応答"}
...
async def main():
    ...
    if not os.path.exists(MANIFEST):
        log("[ERROR] n5_listening_manifest.json not found next to this script.")
        sys.exit(1)

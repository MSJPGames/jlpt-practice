# -*- coding: utf-8 -*-
"""
BJT 第2部 聴読解 練習用の音声を作るスクリプト（edge-tts 使用）
- bjt_choudokkai_data.js を読み、各問題の音声を audio/bjt_<id>.mp3 として作ります。
- 声：ナレーター=女性A、女=女性B、男=男性（自動で選択）。
- 追加ソフトは不要（ffmpegも不要）。edge-tts だけ使います。

使い方（Windows）:
  1) このファイルと bjt_choudokkai_data.js を同じフォルダに置く
  2) コマンドで:  py -m pip install edge-tts
  3)             py make_bjt_audio.py
  4) 出来た audio フォルダを、HTMLと同じ場所に置いてアップロードする

ログは _choudokkai_audio_log.txt にも書き出します。
"""
import os, re, json, sys, asyncio, traceback

HERE = os.path.dirname(os.path.abspath(__file__))
DATA_JS = os.path.join(HERE, "bjt_choudokkai_data.js")
OUT_DIR = os.path.join(HERE, "audio")
LOG = os.path.join(HERE, "_choudokkai_audio_log.txt")
...
def load_items():
    ...
    i = s.index("window.BJT_CHOUDOKKAI")
...
async def main():
    ...
    for idx, it in enumerate(items, 1):
        out = os.path.join(OUT_DIR, "cd_%s.mp3" % it["id"])
        try:
            parts = []
